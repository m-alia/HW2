 ...
 */
 ...
 public class BinaryTree {
     ...
     private void replaceValueHelper(Node node, int oldVal, int newVal) {
        // logic to check every node 
        if (node.left != null) {
            replaceValueHelper(node.left, oldVal, newVal);
        } // go as far left as possible, uses recursion
        if (node.data == oldVal) {
            node.data = newVal;
        } // checks if current node has oldVal, if so, replace the data of the current node 
        if (node.right != null) {
            replaceValueHelper(node.right, oldVal, newVal);
        } // goes right if no left possibilities, and there is room to go right
     ...
     private int findMinHelper(Node node) {
        ...
        // if tree is empty, return MAX_VALUE

        int minimum = node.data; 
        int leftMin = findMinHelper(node.left);
        int rightMin = findMinHelper(node.right);
        // two variables, one traverses all left nodes, other traverses all right
        // uses recursion, and finds the min for the right and left subtrees
        ...
        // compare left and right mins to root node
        // if smaller, replace the minimum with new min
     ...
     private int nodesGTHelper(Node node, int val) {
        ...
        int greaterThan = 0;
        // start greaterThan count at 0 

        if (node.left != null) {
            greaterThan += nodesGTHelper(node.left, val);
        } // traverse through left side, uses recursion
        if (node.data > val) {
            greaterThan++;
        } // add to greaterThan if node is greater than val
        if (node.right != null) {
            greaterThan += nodesGTHelper(node.right, val);
        } // traverse through right side, uses recursion

        return greaterThan;
        // return the final count of nodes greater than val
     }
     ...
     private int[] averageHelper(Node n) {
        ...
        int[] leftSum = averageHelper(n.left); // traverse left subtree, uses recursion
        int[] rightSum = averageHelper(n.right); // traverse right subtree, uses recursion

        int sum = leftSum[0] + rightSum[0] + n.data; // add left and right sums plus data from previous iteration
        int count = leftSum[1] + rightSum[1] + 1; // add left and right count plus one for each iteration

 
         return new int[]{sum, count};
         // return final array of sum and count
     }
 }